...
Fl_Plot::Fl_Plot(int x, int y, int w, int h, const char* label)
    ...
    display_labels_btn_ =
        new Fl_Button(0, 0, 0, 0, getToggleButtonText().c_str());
...
void Fl_Plot::draw() {
    ...
    display_labels_btn_->resize(btn_pos.first, btn_pos.second, btn_size.first,
                                btn_size.second);
}
...
void Fl_Plot::drawText(const std::string& text, const PointXY& position,
                       int angle) {
    if (angle == 0) {
        ...
        fl_draw(
            "", 0,
            0); // Needed to draw rotated text, don't know why. Maybe a bug....
