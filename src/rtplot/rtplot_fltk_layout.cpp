...
RTPlotFLTKLayout::RTPlotFLTKLayout(int x, int y, int w, int h, const char* l)
    : Fl_Table(x, y, w, h, l), RTPlotLayout() {
    col_header(0);
    col_resize(1);
    col_header_height(0);
    row_header(0);
...
void RTPlotFLTKLayout::setPlots(
    std::vector<std::shared_ptr<RTPlotCore>> widgets, size_t num_rows,
    size_t num_cols) {
...
void RTPlotFLTKLayout::draw_cell(TableContext context, int R, int C, int X,
                                 int Y, int W, int H) {
    switch (context) {
